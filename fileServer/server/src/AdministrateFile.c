...
int ConductCommandRemove(int ClientFd,char para[30],char path[40])
{
	...
	if(ret < 0)
	{
		...
		sprintf(msg,"Not Have The File");
		ret = send(ClientFd,msg,sizeof(msg),0);
		if(ret < 0)
		{
			perror("in remove function send 'not have the file error'");
			return -3;
		}
...
//	char *ptr[20];
	...
	{
		memset(buf,0,sizeof(buf));
		send(ClientFd,"begin upload",sizeof("begin upload"),0);
...
int AdminiFunc(int ClientFd,char UserName[40]) 
{
	...
	while(1)						//循环的处理刚上线的用户发来的命令
	{
		...
		while(ptr[i] != ' ')
		{
			command[i] = ptr[i]; 
			i++;
			if(!strcmp(command,"ls") || !strcmp(command,"exit"))
				break;
		}
		...
		else if(!strcmp(command,"upload"))
		{
			...
			if(ret < 0)
			{
				printf("client upload the file alreay exsit");
				continue;
			}
		}	
		...
		{
			printf("the client [%d] offline exit\n",ClientFd);
			break;
		}
